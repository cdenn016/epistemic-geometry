...
# transport_cache.py
def set_morphism_bases(ctx, agent, Phi0, Phit0, *, allow_overwrite: bool = False):
    """Write (Phi_0, Φ̃_0) into persistent cache; skip if already set unless allow_overwrite=True."""
    ns  = ctx.cache.nsp("morph_base")
    key = _bases_key(agent)
    if (not allow_overwrite) and (key in ns):
        return
    ns[key] = (np.asarray(Phi0,  np.float32), np.asarray(Phit0, np.float32))
